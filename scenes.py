...
def cliffside():
  ...
  disp.wrap(
    disp.jobswitch(data.getter("job"), [
  ...
  option_list = disp.jobswitch(data.getter("job"), [
    [
    ...
    [
      "Continue climbing.", "Stop to smell the flowers.",
      "Give up. Go home, maybe curl up with the newest issue of Locksmith's Digest."
    ],
    [
      "Continue ascending.", "Stop to smell the flowers.",
      "Give up. Go home, maybe curl up with a book.",
      "What was that about a bubble spell?"
    ]
  ])
  ...
  while True:
    ...
    if prompt_chosen == "Stop to smell the flowers.":
      ...
      if data.getter("job") == 3:
      ...
      elif data.getter("job") == 1 or 2:
        if data.getter("job") == 1:
          disp.wrap(
            """  The muscles in your forearms work overtime as you momentarily relax the rest of your upper body. Your horseshoes find purchase on a big mossy stone, providing a welcome hoofhold."""
          )
          input()
        elif data.getter("job") == 2:
          disp.wrap(
        ...
        disp.wrap(
          """  On the way up, your nervous system had dulled itself to the world to protect you. This rest, while needed, is slowly allowing *feeling* back into your sensory receptors."""
        )
        input()
      ...
      input()
      if data.getter("job") == 1:  # Knight text
        disp.wrap("""
      ...
      elif data.getter("job") == 2:  # Thief text
        disp.wrap("""
      ...
      elif data.getter("job") == 3:  # Wizard text
        disp.wrap("""
      ...
      if data.getter("job") == 1:
        ...
        disp.wrap(
          """  The sun is blocked out by the tower which looms closer with each passing second, casting a variety of long shadows. Your long ears bake in the sun, but with a keen eye and astounding balance, you make steady progress.
        """)
      elif data.getter("job") == 3:
        disp.wrap(
